...
{
  Gallery_utils::DeleteFiles(str_replace('/', DIRECTORY_SEPARATOR, Gallery_utils::DefaultGalleryThumbsPath()), '*', false);
  $galleries = Gallery_utils::GetGalleries();
  foreach ($galleries as $gallery)
  {
    $dir = str_replace('/', DIRECTORY_SEPARATOR, Gallery_utils::DefaultGalleryPath() . $gallery['filepath'] . (empty($gallery['filepath']) ? '' : '/') . ($gallery['filename'] == "Gallery/" ? '' : $gallery['filename']));
