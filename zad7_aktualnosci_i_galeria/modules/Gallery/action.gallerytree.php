...
if (Gallery_utils::CleanFile($params['dir']) !== FALSE)
{
	...
	if ($result && $result->RecordCount() > 0)
	{
		while ($row = $result->FetchRow())
		{
			...
			if (empty($row['thumbname']) || !file_exists(Gallery_utils::DefaultGalleryPath() . $row['thumbpath'] . $row['thumbname']))
			{
				$rec->thumb = $folderpath;
			}
			elseif ($templateprops['thumbwidth'] > 0)
			{
				$rec->thumb = Gallery_utils::DefaultGalleryThumbsUrl() . $row['defaultfile'] . '-' . $templateprops['templateid'] . substr($row['thumbname'], strrpos($row['thumbname'], '.'));
				$originalimage = Gallery_utils::DefaultGalleryPath() . $row['thumbpath'] . $row['thumbname'];
			}
			else
			{
				$rec->thumb = Gallery_utils::DefaultGalleryUrl() . str_replace('%2F', '/', rawurlencode($row['thumbpath'] . IM_PREFIX . $row['thumbname']));
				$originalimage = Gallery_utils::DefaultGalleryPath() . $row['thumbpath'] . $row['thumbname'];
			}
