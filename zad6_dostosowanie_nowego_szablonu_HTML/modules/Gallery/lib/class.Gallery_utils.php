...
class Gallery_utils {
	...
	public static function Getdirfiles($path, $recursive) {
		...
		if ($handle = opendir(str_replace('/', DIRECTORY_SEPARATOR, self::DefaultGalleryPath() . $path)))
		{
			while (false !== ($file = readdir($handle)))
			{
				...
				if (substr($file, 0, 1) != "." && substr($file, 0, strlen(IM_PREFIX)) != IM_PREFIX && ((in_array(strtolower($ext), $allowext) && in_array(strtolower($ext), $maxext)) || is_dir(self::DefaultGalleryPath() . $path . $file)))
				{
	...
	public static function DeleteGalleryDB($path, $gid) {
		...
		if ($result && $result->RecordCount() > 0)
		{
			...
			while ($row = $result->FetchRow())
			{
				...
				// delete thumbs created for this image
				self::DeleteFiles(self::DefaultGalleryThumbsPath(), $row['fileid'] . '-*', false);
				// delete original files and IM-thumbs
				self::DeleteFiles(str_replace('/', DIRECTORY_SEPARATOR, self::DefaultGalleryPath() . (empty($row['filepath']) ? '' : $row['filepath'] . '/') . $row['filename']));
				self::DeleteFiles(str_replace('/', DIRECTORY_SEPARATOR, self::DefaultGalleryPath() . $row['filepath']), IM_PREFIX . $row['filename'], false);
				//Update search index
   ...
   */
	...
	public static function CleanFile($filename) {
		...
		$galleryrootpath = realpath(self::DefaultGalleryPath());
		$filepath = realpath(str_replace('/', DIRECTORY_SEPARATOR, self::DefaultGalleryPath() . $filename));
